...
define( function( require, exports, module ) {
	...
	Index.prototype._load = function() {
		...
		function rank(i) {
			...
			$.get("http://localhost:8080/bruceweb/UpAndDownloadServlet",{method:"getRank",singer_name:i},function(res) {
				var json = $.parseJSON(res);
				var html = '';
				$.each(json, function (index, value) {
					html += '<dd data-id="' + value.music_id + '">' +
								'<span>' + (index+1) + '</span>' +
								'<a target="_blank" href='+'\"/bruceweb/music/WEB-INF/view/result.html?id='+value.music_id+'\">'+value.name+'</a>' +
								'<div class="dd-oper">' +
	...
	Index.prototype._bind = function() {
		...
		$('.wrap .main-rank').on('click', this.rankALLPlay, function () {
			...
			$.ajax({
				url: "http://localhost:8080/bruceweb/UpAndDownloadServlet",
				type: "POST",
				data: {
					method:"getmusic"
				},
			 ...
			 */
				...
				$.get("http://localhost:8080/bruceweb/UpAndDownloadServlet",{method:"getMInfo",id:addID},function(res) {
