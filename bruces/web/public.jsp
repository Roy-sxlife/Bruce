...
<head>
    <title>热门动态</title>
    <link rel="stylesheet" href="${pageContext.request.contextPath}/aliyun/love/iconfont.css">
...
<%--    循环显示这一页的文章--%>
    ...
    <div class="publish-content">
        ...
        <p style="margin-top: 38px;float: left;margin-left: 20px;"> <a style="text-decoration: none" target="_blank" href="${pageContext.request.contextPath}/CenterServlet?method=ShowAll&user_Id=${status.user.id}">${status.user.username}<a/></p>
        ...
        <p style="width: 620px;margin: 30px auto;line-height: 30px;font-size: 16px;color: #333;">
            ...
            <img style="width: 200px;height: 250px;" controls onerror="javascript:this.style.display = 'none'" loop src="${pageContext.request.contextPath}/resources/picture/${status.mood}">
                ...
                <ul class="thefunction">
                    ...
                    <c:choose>
                        <c:when test="${list.get(status.status_id)}"><a href="javascript:;" onclick="follow(${status.user.id},${status.status_id})" style="text-decoration: none;color: #666666;"><li style="margin-left: 95px;position: relative;"><img id="${status.status_id}attention${status.user.id}" src="${pageContext.request.contextPath}/resources/img/love.png" style="width: 25px;height: 25px;position: absolute;right:20px;top: -2px;"></li></a> </c:when>
                        <c:otherwise><a href="javascript:;" onclick="follow(${status.user.id},${status.status_id})" style="text-decoration: none;color: #666666;"><li style="margin-left: 95px;position: relative;"><img id="${status.status_id}attention${status.user.id}" src="${pageContext.request.contextPath}/resources/img/afv.png" style="width: 25px;height: 25px;position: absolute;right:20px;top: -2px;"></li></a></c:otherwise>
                    </c:choose>
...
<%--        评论列表区--%>
                <div id="Remarksection${status.status_id}" style="margin-left: 30px;display: none"  >
                    <c:forEach items="${theRemarks.get(status.status_id)}" var="remark">
                    <div class="publish-content">
...
<%--            发布评论--%>
                <div id="Replycomment${status.status_id}" style="display: none;margin-top: -40px;margin-left: 30px">
                    <div class="backgroud">
    ...
    <p style="text-align: center;color: #666;">
        <!-- 分页选择链接 -->
        <c:if test="${requestScope.p3.totalPage!=0}">
    <div >
    <span >当前第 ${requestScope.p3.currentPage} 页，总共 ${requestScope.p3.totalPage} 页
    </span>
        <a href="StatusServlet?method=ShowAll&currentPage=1" >首页</a>
        <!-- 利用el表达式的三目运算符进行判断 -->
        <a href="StatusServlet?method=ShowAll&currentPage=${(requestScope.p3.currentPage==1)?1:requestScope.p3.currentPage-1}" >上一页</a>
        <a href="StatusServlet?method=ShowAll&currentPage=${(requestScope.p3.currentPage==requestScope.p3.totalPage)?requestScope.p3.totalPage:requestScope.p3.currentPage+1}" >下一页</a>
        <a href="StatusServlet?method=ShowAll&currentPage=${requestScope.p3.totalPage}" >尾页</a>
    </div>
    </c:if>
    </p>
</div>
...
<script type="text/javascript">
    ...
    function ShowReply(name,a) {
        ...
        xmlHttpRequest.open("post","${pageContext.request.contextPath}/StatusServlet",true);
   ...
   function follow(f,s) { //关注和取消关注
       ...
       xmlHttpRequest.onreadystatechange=function(){
           if(xmlHttpRequest.readyState == 4&&xmlHttpRequest.status == 200) {
               ...
               if (ifFollow=="true"){
                   document.getElementById(status_id+"attention"+follow_id).src="${pageContext.request.contextPath}"+"/"+"resources"+"/"+"img"+"/"+"love.png";
               }else if(ifFollow=="false"){
                   document.getElementById(status_id+"attention"+follow_id).src="${pageContext.request.contextPath}"+"/"+"resources"+"/"+"img"+"/"+"afv.png";
               }
           }
